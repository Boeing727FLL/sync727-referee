...
/**
 * The three stages (draft -> critique -> final) inside the single request.
 * They used to be three separate calls; now the model runs the draft and the
 * critique inside its <think> block and writes only the final answer outside
 * it. The critique and final texts are the old CRITIQUE_PROMPT / FINAL_PROMPT
 * (plus the old photo addendum), kept whole.
 */
export const ANSWER_PROMPT = `

[שלושה שלבים בתוך הבקשה הזו - חובה לבצע את שלושתם לפי הסדר]:

[שלב 1 - טיוטה, בתוך בלוק ה-<think>]:
כתוב טיוטת תשובה מלאה: זיהוי המשימה, החוקים הרלוונטיים עם מספרי הכללים, הפסיקה וחישוב הניקוד, לפי הוראות החשיבה שלמעלה.

[שלב 2 - מערכת בקרה קוגניטיבית עילאית Claude Fable 5 - בקורת עצמית עוינת (Adversarial Critique), בתוך בלוק ה-<think>, אחרי הטיוטה]:
...
- ביקורת חזותית: אם צורפה תמונת משתמש (USER PHOTO), בדוק שהזיהוי החזותי בטיוטה - משימה, מיקום, מגע, ניקוד - תואם את מה שבאמת רואים בתמונה. אם לא, תקן.
נסח בקצרה את מסקנות הביקורת והתיקונים שחובה לבצע.

[שלב 3 - הפקת התשובה הסופית, מחוץ לבלוק ה-<think>]:
...
תקן בתשובה הסופית כל מה שהביקורת מצאה. אל תכתוב את הטיוטה או את הביקורת מחוץ לבלוק ה-<think>.`;
