      ...
      <div className={`flex flex-col gap-1.5 md:gap-2 min-w-0 ${isUser ? 'max-w-[85%] md:max-w-[70%] items-end' : 'min-w-0 max-w-[88%] md:max-w-[80%] lg:max-w-[72rem]'}`}>
        ...
        <div className={`relative overflow-hidden transition-all duration-500 ${isUser ? 'bg-[#0a84ff] text-white rounded-[20px] rounded-se-[8px] shadow-[inset_0_1px_0_rgba(255,255,255,0.22),0_6px_20px_rgba(10,132,255,0.3)] px-3.5 py-2.5 md:px-4 md:py-3' : 'bg-[#04060c]/45 border border-white/[0.14] backdrop-blur-xl backdrop-saturate-150 text-slate-100 rounded-[18px] rounded-ss-[8px] shadow-[inset_0_1px_0_rgba(255,255,255,0.18),0_8px_28px_rgba(0,0,0,0.3)] px-4 py-3 md:px-5 md:py-3.5'}`}>
          ...
          <div className={`text-[15px] md:text-[16px] leading-relaxed ${isUser ? 'font-medium' : 'font-normal'}`}>
            {isUser ? <div className="whitespace-pre-wrap break-words">{message.quote && <div className="mb-1.5 rounded-lg border-r-2 border-white/25 bg-white/[0.04] px-2.5 py-1.5 text-xs text-white/60 line-clamp-3 text-right">{message.quote}</div>}{message.text}</div>
              : <div className="prose prose-invert max-w-none break-words prose-p:leading-relaxed prose-p:my-2 prose-p:text-slate-100 prose-headings:font-bold prose-headings:text-white prose-headings:mt-3 prose-headings:mb-1.5 prose-a:text-[#7FB8EC] prose-strong:text-[#FFC400] prose-ul:list-disc prose-ol:list-decimal prose-li:my-1 prose-li:text-slate-200 rtl:text-right"><Suspense fallback={<span>{text}</span>}><MarkdownMessage components={{ em: ({ children, ...props }) => { const value = typeof children === 'string' ? children : Array.isArray(children) && children.length === 1 && typeof children[0] === 'string' ? children[0] : null; return value === '▍' ? <span className="typewriter-cursor" aria-hidden>▍</span> : <em {...props}>{children}</em>; } }}>{typewriting ? text + '\u200B*\u258D*' : text}</MarkdownMessage></Suspense></div>}
          </div>
        </div>
        {!isUser && index > 0 && <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ ...MOTION.filmReveal, delay: liveAnswer ? 0 : 0.12 }} className="flex w-fit items-center gap-1.5 rounded-xl border border-white/[0.12] bg-[#04060c]/45 px-1 py-1 backdrop-blur-xl backdrop-saturate-150 shadow-[inset_0_1px_0_rgba(255,255,255,0.12),0_6px_20px_rgba(0,0,0,0.28)]">
          <button onClick={() => onCopy(text)} className="text-[11px] md:text-xs font-bold text-white/55 hover:text-white transition-colors px-2.5 py-1.5 rounded-lg hover:bg-white/[0.08] cursor-pointer flex items-center gap-1.5"><Copy className="w-3.5 h-3.5" />{t('chat.copy')}</button>
          {!liveAnswer && <button onClick={() => onReply(text)} className="text-[11px] md:text-xs font-bold text-white/55 hover:text-white transition-colors px-2.5 py-1.5 rounded-lg hover:bg-white/[0.08] cursor-pointer flex items-center gap-1.5"><Reply className="w-3.5 h-3.5" />{t('chat.reply')}</button>}
        </motion.div>}
      </div>
    </motion.div>
  );
