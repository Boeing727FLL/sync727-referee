...
export default function ChatMessageRow({ view, userPicture, userName, onCopy, onReply, t }: Props) {
  ...
  if (thinking) return (
    <motion.div layout layoutId={`message-${index}`} initial={{ opacity: 0, scale: 0.985, clipPath: 'inset(0 0 100% 0 round 16px)' }} animate={{ opacity: 1, scale: 1, clipPath: 'inset(0 0 0% 0 round 16px)' }} exit={{ opacity: 0, scale: 0.99, clipPath: 'inset(0 0 100% 0 round 16px)' }} transition={MOTION.morph} className="flex gap-2.5 md:gap-3">
      ...
      <div className="bg-white/[0.02] border border-white/[0.07] px-4 py-3 rounded-2xl flex flex-col items-center gap-2 max-w-[85%] md:max-w-[75%]">
        <ThinkIndicator />
    ...
    <motion.div layout layoutId={`message-${index}`} initial={{ opacity: 0, y: 18, scale: 0.985, clipPath: 'inset(0 0 100% 0 round 16px)' }} animate={{ opacity: 1, y: 0, scale: 1, clipPath: 'inset(0 0 0% 0 round 16px)' }} exit={{ opacity: 0, y: -6, scale: 0.99, clipPath: 'inset(0 0 100% 0 round 16px)' }} transition={MOTION.filmReveal} className={`flex gap-2.5 md:gap-3.5 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
      <div className="w-8 h-8 md:w-9 md:h-9 rounded-full flex items-center justify-center shrink-0 overflow-hidden">
        {isUser ? (userPicture ? <img src={userPicture} alt="" className="w-full h-full object-cover rounded-full ring-1 ring-white/20" /> : <div className="w-full h-full rounded-full bg-white/10 flex items-center justify-center"><span className="text-xs md:text-sm font-black text-white">{(userName || 'U').trim().charAt(0)}</span></div>)
          : <div className={`w-full h-full rounded-full bg-white overflow-hidden transition-all duration-500 ${liveAnswer ? 'ring-2 ring-[#ff7a66]/70' : 'ring-1 ring-white/15'}`}><img src="/logoref.png" alt={t('app.title')} className="w-full h-full object-contain" /></div>}
      </div>
      <div className={`flex flex-col gap-1.5 md:gap-2 min-w-0 ${isUser ? 'max-w-[85%] md:max-w-[70%] items-end' : 'min-w-0 max-w-3xl'}`}>
        <div className={`relative overflow-hidden transition-all duration-500 ${isUser ? 'bg-[rgba(159,216,198,0.09)] border border-[rgba(159,216,198,0.18)] text-white rounded-2xl px-3.5 py-2.5 md:px-4 md:py-3' : 'bg-white/[0.02] border border-white/[0.07] text-slate-100 rounded-2xl px-4 py-3 md:px-5 md:py-4'}`}>
          {!isUser && <div className="flex items-center gap-1.5 mb-1.5 md:mb-2"><span className="text-[10px] md:text-[11px] font-black tracking-wide text-[#ff7a66]/90 flex items-center gap-1.5"><span aria-hidden className="w-[4px] h-[4px] rounded-full bg-[#ff7a66]" />{t('chat.refereeTag')}</span>{text.includes('שריקה') && <span className="text-[10px] md:text-[11px] font-black text-[#ff7a66]">{t('chat.foulTag')}</span>}</div>}
