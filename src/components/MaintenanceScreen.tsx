/**
 * MaintenanceScreen — full-takeover "we're working" page (work mode ON).
 *
 * WHAT: everyone except the owner sees this instead of the app while work
 * mode is on: brand ribbon, glowing emblem, three explainer cards, and a
 * credit line. No buttons, nothing to do but wait — the screen vanishes by
 * itself the moment work mode turns off (realtime flag, no refresh needed).
 *
...
/** Fixed vivid backdrop: field photo, veil, grid, breathing FIRST glows. */
function Backdrop() {
  return (
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        <h2 className="text-sm font-black text-white leading-tight mb-1">{card.title}</h2>
        <p className="text-xs text-slate-400 leading-relaxed font-medium">{card.text}</p>
      </div>
    </motion.div>
  );
}

/** Three pulsing dots: the "still working" heartbeat. */
function TypingDots() {
  return (
