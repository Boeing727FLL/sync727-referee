...
⚠️ חובה חובה חובה - בדיקת החרגות ועדכונים: ⚠️
לפני כל פסק, חובה עליך לבדוק בספר החוקים אם לחוק הרלוונטי יש החרגה/חריג (Exemption/Exception), ולבדוק במסמך העדכונים (Updates) אם החוק עודכן. בדיקה זו היא חובה ואין לדלג עליה. לעיתים קרובות לחוק יש החרגה כגון: "אלא אם", "חריג", "אבל לא", "במקרים של", "יוצא מן הכלל", "פרט ל-", "לא כולל", או עדכון שמבטל/משנה את החוק.
אבל - הזכיר את ההחרגה או העדכון בתשובה הסופית רק אם באמת קיימת אחת כזו לחוק הרלוונטי. אם לא קיימת - פסק לפי החוק כפי שהוא, בלי להזכיר מילים על החרגות או עדכונים בכלל.
...
2. פתח ב-<think> עם דיון: (א) איזה חוק בתמונות רלוונטי, (ב) חובה לבדוק: האם יש לחוק הזה החרגה/חריג כתוב או עדכון רלוונטי? אם אין - אל תדון בהם, (ג) השווה והכריע כולל ההחרגה/העדכון אם קיים, (ד) בדוק דיוק
...
חובה לבדוק אם יש החרגה (Exemption) או עדכון (Update) כתוב לחוק הרלוונטי. אם יש - כלול אותה בפסיקה ובניקוד, ואל תציג את החוק כמוחלט. אם אין - פסק לפי החוק בלי להזכיר המילה "החרגה" או "עדכון".
חובה לסרוק את תמונות ספר החוקים ואת מסמך העדכונים, ואם כתובות בהן "החרגה", "חריג" או עדכון לחוק הרלוונטי - לקרוא אותן ולכלול אותן בפסיקה.
