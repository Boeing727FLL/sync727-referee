...
import {
  TEAM_STORAGE_KEY, createTeam, getActiveTeamId, joinTeam,
  subscribeTeamMembers, subscribeTeamQuestions,
  type TeamMember, type TeamQuestion, type TeamWorkspace,
...
export function useTeamWorkspace(isOpen: boolean, currentUser: TeamMember, onTeamChange: (team: TeamWorkspace | null) => void) {
  ...
  useEffect(() => {
    ...
    if (!id) return;
    joinTeam(id, currentUser).then(value => {
  ...
  const open = (value: TeamWorkspace) => {
    localStorage.setItem(TEAM_STORAGE_KEY, value.id);
    setTeam(value);
    onTeamChange(value);
  };
  ...
  const create = async (name: string) => {
    ...
    setBusy(true);
    try {
      open(await createTeam(name, currentUser));
      return true;
    } catch (error) {
      notify(error instanceof Error ? error.message : 'לא הצלחתי ליצור את הקבוצה. נסו שוב.');
      return false;
    } finally {
      setBusy(false);
    }
  ...
  const join = async (code: string) => {
    ...
    setBusy(true);
    try {
      const value = await joinTeam(code, currentUser);
      if (!value) {
        notify('לא נמצאה קבוצה עם הקוד הזה.');
        return false;
      }
      open(value);
      return true;
    } catch (error) {
      notify(error instanceof Error ? error.message : 'לא הצלחתי להצטרף לקבוצה. נסו שוב.');
      return false;
    } finally {
      setBusy(false);
    }
