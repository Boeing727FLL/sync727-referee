...
export function describeRequestFile(file: { key: string; url: string; isRulebook: boolean; actualFile?: File }): RequestFileDescriptor {
  ...
  return {
    ...
    isText: Boolean(file.actualFile?.type?.startsWith('text/') || /\.(txt|md|json|xml)$/i.test(fileName)),
...
export function textRulebookLabel(fileName: string, text: string): string {
  return `
--- RULEBOOK TEXT | FILE: ${fileName} ---
${text}
--- END RULEBOOK TEXT ---
`;
}
